(() => {
  ...
  async function getVerseData(chapterNumber, verseNumber) {
    ...
    return {
      ...
      verse_number: verse,
      // Using requested verse, even if it's part of a merged block
      text: verseEntry.verse_text.replace(/\n+$/g, "").replace(/\n/g, "<br/>"),
      // Basic formatting
      translations: [
        {},
        {},
        { description: verseEntry.translation }
        // API translation is usually at index 2
      ],
      transliteration: verseEntry.devanagari,
      // Using devanagari as a fallback for transliteration
      word_meanings: verseEntry.synonyms
    };
  }
  var chapterSelect = document.getElementById("chapter");
  var verseSelect = document.getElementById("verse");
  ...
  async function showRandomVerse() {
    ...
    const randomVerseNumber = Math.floor(Math.random() * maxVerses[randomChapterNumber - 1]) + 1;
  ...
  async function showPreviousVerse() {
    ...
    if (verseNumber > 1) {
      ...
      if (chapterNumber > 1) {
        chapterSelect.value = parseInt(chapterSelect.value) - 1;
        verseSelect.innerHTML = "";
        for (let i = 1; i <= bhagavadGitaChapters[chapterNumber - 2].verses; i++) {
          const option = document.createElement("option");
          option.text = "Verse " + i;
          option.value = i;
          verseSelect.add(option);
        ...
        verseSelect.innerHTML = "";
        for (let i = 1; i <= bhagavadGitaChapters[bhagavadGitaChapters.length - 1].verses; i++) {
          const option = document.createElement("option");
          option.text = "Verse " + i;
          option.value = i;
          verseSelect.add(option);
        }
        verseNumber = bhagavadGitaChapters[bhagavadGitaChapters.length - 1].verses;
        verseSelect.value = verseNumber;
      }
    }
    try {
  ...
  document.getElementById("previousButton").addEventListener("click", showPreviousVerse);
  function updateUIWithAnimation(data, direction = "top") {
    ...
    if (data.word_meanings) {
      formattedWordMeanings = data.word_meanings.split(";").map((part) => {
    ...
    newSpeakButton.addEventListener("click", () => {
      chrome.tts.speak(data.text, {
        lang: "sa",
        // Note: The old text had Sanskrit but Chrome might not support 'sa' flawlessly everywhere
        rate: 1,
        pitch: 1
      });
    });
    ...
    const isBookmarked = favorites.some(
      (f) => f.chapter === data.chapter_number && f.verse === data.verse_number
    );
    ...
    bookmarkButton.onclick = () => {
      ...
      const existingIndex = favorites.findIndex(
        (f) => f.chapter === verseInfo.chapter && f.verse === verseInfo.verse
      );
      if (existingIndex === -1) {
